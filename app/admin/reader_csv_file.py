...
def verifica_prazo(*args, **kwargs):
    with db.engine.begin() as conn:
        try:
            print('prazooo', kwargs.get('sku'), kwargs.get('prazo'))
            query_produto = (text("""SELECT [SKU],[PrazoProducao]
                FROM [HauszMapa].[Produtos].[ProdutoPrazoProducFornec]
                WHERE [SKU] = '{}'""".format(kwargs.get('sku'))))
            exec_produtos = conn.execute(query_produto).all()
            print('prazooo', kwargs.get('sku'), kwargs.get('prazo'))
            for exec in exec_produtos:
                if kwargs.get('sku') == 0:
                    return exec['PrazoProducao']
                
                return kwargs.get('prazo')
        except:
            print("erro prazo")
    ...
    with db.engine.begin() as conn:
        try:
            stmt = (
                ...
                PrazoProducaoAlterado=kwargs.get('prazo'),ValorAnterio = kwargs.get('saldoanterior'),ValorAlterado = kwargs.get('saldo')
...
def call_procedure_prazo_hausz_mapa(f):
    ...
    def update_prazo(*args, **kwargs):
        prazo = verifica_prazo(sku = kwargs.get('sku'), prazo = kwargs.get('prazo'))
...
def select_produtos_hausz_mapa(*args, **kwargs):
    with db.engine.begin() as conn:
        query_produto = (text("""
            ...
            ,pprazof.PrazoProducao,pmarca.Marca,pbasico.[IdProduto]
        ...
        try:
            for produtos in execquery:
                produtos_dict = {
                    ...
                    'PRAZOANTERIOR':produtos['PrazoProducao'],
...
file = '/home/debian/Área de trabalho/flaskapprefatorado/app/uploads/csv/'
delete  = '/home/debian/Área de trabalho/flaskapprefatorado/app/uploads/csv/'

def delete_all_files():
    valor = os.listdir(file)
    for val in valor:
        if '.csv' in val:
            print(val)
            os.remove(os.path.join(delete, val))
